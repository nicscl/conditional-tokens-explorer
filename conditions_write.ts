...
export function assignQuestionToCondition(condition: Condition, questionId: string): void {
  ...
  let categoryId = question.get("category");
  if (categoryId && categoryId.toString() !== '') {
    // Process valid categoryId
    let category = Category.load(categoryId.toString());
    if (!category) {
      category = new Category(categoryId.toString());
